...
export default function MagicRings({
  ...
  const config = useMemo(
    () => ({
      color,
      colorTwo,
      speed: reducedMotion ? 0.12 : speed,
      ringCount,
      attenuation,
      lineThickness,
      baseRadius,
      ...
      opacity,
      blur,
      noiseAmount: reducedMotion ? 0 : noiseAmount,
      rotation,
      ringGap,
      fadeIn,
      fadeOut,
  ...
  const containerClass = className ? `magic-rings-container ${className}` : "magic-rings-container";

  return <div ref={mountRef} className={containerClass} style={blur > 0 ? { filter: `blur(${blur}px)` } : undefined} aria-hidden="true" />;
}
